                    ...
                    <ul>
                        {conversations.map((conversation) => (
                            <li
                                ...
                                className={`cursor-pointer p-2 rounded mb-2 ${currentConversation?.conversation_id === conversation.conversation_id
                                    ? 'bg-blue-100'
                                    : 'hover:bg-gray-200'
                                    }`}
                            >
                                <div className="flex justify-between">
                                    <strong>{conversation.receiver_id}</strong>
                    ...
                    {currentConversation ? (
                        <div>
                            <h3 className="text-2xl font-semibold mb-4">
                                Conversation with {currentConversation.receiver_id}
                            </h3>
                            <ul className="mb-4 max-h-72 overflow-y-auto">
                                {messages.map((message) => (
                                    // Create a unique key using a combination of fields
                                    <li key={`${message.chat_id}-${message.createdAt}`} className="mb-2">
